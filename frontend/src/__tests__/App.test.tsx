...
/**
 * Behaviour through the DOM. Every expected value is read from the contract;
 * nothing below hand-writes a payload or a result.
 */
...
/** The key's accessible name: its aria-label, or its glyph for the digits. */
...
/**
 * The keystrokes that enter a number, or null when the keypad cannot express it.
 *
 * Two fixture operands are genuinely beyond a keypad: anything needing an
 * exponent (`1e308`) and anything with more than nine mantissa digits. Those
 * cases are swept separately rather than being quietly dropped.
 */
function keysForNumber(value: number): readonly string[] | null {
  // -0 is typed as 0; the wire form of both is the same bytes, and the sign key
  // is deliberately inert on zero.
...
/** The full key sequence for a fixture, or null when it is beyond keypad entry. */
...
/** Drives the keypad exactly as a user would, from a fixture's own request. */
...
/**
 * A client that answers each successive request from a script, so a run of
 * operations can be driven end to end. An unscripted request is a test bug and
 * says so rather than quietly reusing the last answer.
 */
...
/**
 * Every preview case the typed UI can produce, across both milestones. The one
 * case carrying `testFault` is excluded: it is byte-identical to add-positive
 * and reachable only by name, so it is exercised in its own describe block.
 */
...
describe('the keys that would otherwise compute in the browser', () => {
  ...
  it('sends sqrt with exactly one operand and nothing stale beside it', async () => {
    ...
    // A previous entry and an abandoned operator must not ride along.
  ...
  it('toggles the sign by editing the display and issues no request', async () => {
    ...
    // The edited string is what reaches the wire, as an ASCII minus.
...
describe('every preview operation produces the fixtures expected result', () => {
  ...
  it('covers all seven operations across those cases', () => {
    ...
    // And every one of them is reachable by pressing keys.
...
describe('fixtures beyond what a keypad can type', () => {
  it('names them, so the gap cannot widen unnoticed', () => {
    // Exponent notation has no key, and the mantissa cap stops at nine digits.
...
describe('display formatting through the DOM', () => {
  ...
  it('shows sqrt(2) rounded to twelve significant digits', async () => {
    ...
    // The exact binary64 value is offered alongside, not silently discarded.
    expect(liveRegion()).toHaveTextContent('1.4142135623730951');
  });

  it('normalises sqrt of negative zero', async () => {
    // sqrt-negative-zero's request bytes are identical to sqrt-zero's and both
    // expect 0, so the index may answer with either; the display is "0" either
    // way, which is exactly what normalizeNegativeZero pins. The sign key is
    // inert on zero, so -0 cannot be typed in the first place.
    const user = userEvent.setup();
  ...
  it('asserts every expectedDisplay fixture through the DOM, not just as a unit', () => {
    // Guard against this drifting: all five pinned cases are DOM-reachable.
...
describe('error rendering', () => {
  it.each(keypadFailures.map((entry) => [entry.id, entry] as const))(
    ...
    async (_id, entry) => {
      ...
      // The expression line is cleared while an error is shown.
  ...
  it('lets the next keypress dismiss the error', async () => {
    ...
    // A digit dismisses the error and starts a fresh number.
...
describe('injected server fault', () => {
  // The scenario is selected by constructing the client, not through any UI:
  // the application has no runtime affordance for choosing a fixture.
  ...
  it('renders the 500 in wording distinct from a 400', async () => {
    ...
    // A 500 must not be framed as something the user can fix.
    expect(liveRegion()).not.toHaveTextContent(ERROR_COPY.DIVISION_BY_ZERO);
  });
  ...
  it('returns the ordinary 200 for byte-identical input from a healthy client', async () => {
    ...
    // Same operands, same request bytes; only the transport differs.
...
describe('a request is only issued when there is one to issue', () => {
  ...
  it('swaps the armed operator when another is pressed before the second operand', async () => {
    ...
    // Still one operation — this is the mis-tap fix, not a chain.
  ...
  it('leaves a result alone when the sign key is pressed', async () => {
    ...
    // The value line is a rounded rendering of the answer; flipping its sign
    // would quietly substitute that text for the exact number behind it.
    await pressKeys(user, ['sign']);
    expect(liveRegion()).toHaveTextContent('20');
...
describe('a result starts the next operation', () => {
  it('makes the answer the left operand, one operation at a time', async () => {
    ...
    // The operator takes the result as its left operand and says so.
    await pressKeys(user, ['add']);
    expect(liveRegion()).toHaveTextContent('20 +');
    ...
    // Three presses of `=`, three requests: each operation stands alone and
    // nothing is re-evaluated behind the user's back.
  ...
  it('sends the raw result, not the rounded text standing in for it', async () => {
    ...
    // The display gave up four digits to twelve-significant-digit rounding.
    // The request did not, which is the only reason this answers 1 rather than
    // 0.999999999999.
  ...
  it('disables every operation key once the second operand is typed', async () => {
    ...
    // Every operation key is still live: until the second operand is typed the
    // user may change their mind about which operation this is.
    for (const key of operationKeys()) {
      expect(screen.getByRole('button', { name: keyName(key.id) }), key.id).toBeEnabled();
    }

    await pressKeys(user, ['digit-3']);
    // `2 + 3` is on screen. Pressing any operation key now is the only way to
    // ask for a second operation, so every one of them refuses.
    for (const key of operationKeys()) {
      expect(screen.getByRole('button', { name: keyName(key.id) }), key.id).toBeDisabled();
    }
    // The two ways forward stay open.
  ...
  it('lets a unary key replace an armed operation and fire on the left operand', async () => {
    ...
    // The wrong operation key was pressed; one press corrects it rather than
    // costing a Clear and a retype.
    ...
    // One operation, not two: the pending power was dropped, not composed.
  ...
  it('refuses a second operation from the physical keyboard as well', async () => {
    ...
    // The `disabled` attribute is what stops a click; the press handler is what
    // stops a keystroke. Both ask the same predicate, so they cannot disagree.
    expect(liveRegion()).toHaveTextContent('2 +');
    expect(screen.getByRole('button', { name: 'Multiply' })).toHaveAttribute(
      'aria-pressed',
      'false',
...
describe('loading state', () => {
  ...
  it('disables the keypad while a request is in flight and prevents a second one', async () => {
    ...
    // Clear stays live: it is how a slow request is abandoned.
    expect(screen.getByRole('button', { name: 'Clear' })).toBeEnabled();
    ...
    // Backspace bypasses the key table, so it needs its own guard rather than
    // inheriting the disabled attribute.
    await user.keyboard('{Backspace}');
    expect(liveRegion()).toHaveTextContent('2');
  ...
  it('ignores a response that arrives after Clear', async () => {
    ...
    // The physical keyboard is blocked too, or it would route around the
    // disabled buttons and queue a second request.
...
describe('the live region', () => {
  it('is a polite, atomic status region that survives state changes', async () => {
    ...
    // The operand labels used to name this; a hidden heading does it now.
    ...
    // Same element, so assistive technology keeps announcing it.
...
describe('keyboard and focus', () => {
  ...
  it('leaves browser shortcuts alone', async () => {
    ...
    // Ctrl+/ and friends belong to the browser, not to the keypad.
  ...
  it('reaches every key by keyboard alone', async () => {
    ...
    // Enter on a focused key activates that key, not Equals: one press, one key.
  ...
  it('uses real buttons and real labels throughout', () => {
    ...
    // Every rendered key is accounted for, so none is unnamed.
...
describe('failure states are rendered honestly', () => {
  it('says an unmatched calculation is unavailable rather than computing it', async () => {
    ...
    // 41 + 1 is emphatically not answered.
  ...
  it('renders a transport failure reported by the client', async () => {
    ...
    // A transport failure is not the user's fault, so it must not be framed as one.
    expect(liveRegion().querySelector('.display__error--server')).not.toBeNull();
    expect(liveRegion().querySelector('.display__error--user')).toBeNull();
  });

  it('tells the user the service is unreachable when fetch rejects', async () => {
    // The state a user hits when the Go process is not running. This goes
    // through the real createHttpClient, so it exercises the production path
    // from a failed fetch all the way to rendered text.
    const user = userEvent.setup();
    ...
    // The app recovers: the keypad is live again.
    expect(screen.getByRole('button', { name: 'Equals' })).toBeEnabled();
  });
