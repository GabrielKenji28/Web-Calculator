...
/**
 * Guards on the shared contract itself. These fail loudly if the fixture file
 * drifts away from what this app was built against, which is cheaper than
 * discovering it through a broken screen.
 */
describe('contract/fixtures.json', () => {
  ...
  it('offers all seven operations, with sqrt the only arity-1 case', () => {
    // The UI no longer filters by milestone, so this is the button set.
  ...
  it('agrees with the ErrorCode union mirrored from the Go structs', () => {
    // Drift detector: a new server-side code must be added to ERROR_CODES.
  ...
  it('exposes exactly one injected server fault, unreachable by ordinary input', () => {
    ...
    // The collision that forces named selection: identical bytes, different answers.
